--[[--
@module koplugin.wallabagx
]]

-- wallabagx - for Wallabag 2.4+ (or compatible) API servers
-- clach04
-- NOTE for now; uses the same wallabag settings that Wallabag (Wallabag1) uses
-- NOTE for now; has new text WallabagX - may want to remove and revert back to reduce(/remove) translation effort
...
local WallabagX = WidgetContainer:extend{
    name = "wallabagx",
}

function WallabagX:onDispatcherRegisterActions()
    Dispatcher:registerAction("wallabag_download", { category="none", event="SynchronizeWallabagX", title=_("WallabagX retrieval"), general=true,})
end

function WallabagX:init()
    self.token_expiry = 0
    ...
    if self.ui and self.ui.link then
        self.ui.link:addToExternalLinkDialog("25_wallabag2", function(this, link_url)
            return {
                text = _("Add to WallabagX"),
                callback = function()
                    UIManager:close(this.external_link_dialog)
                    this.ui:handleEvent(Event:new("AddWallabagXArticle", link_url))
                end,
            }
        end)
    end
...
function WallabagX:addToMainMenu(menu_items)
    menu_items.wallabagx = {
        text = _("WallabagX"),
        sub_item_table = {
            {
            ...
            },
            {
                ...
                sub_item_table = {
                    {
                        text = _("Configure WallabagX server"),
                        keep_menu_open = true,
                        callback = function()
                    ...
                    {
                        text = _("Configure WallabagX client"),
                        keep_menu_open = true,
                        callback = function()
                    ...
                    },
                    {
                        ...
                        help_text = _("This allow you to write tags in the review field, separated by commas, which can then be sent to WallabagX."),
                        keep_menu_open = true,
                    ...
                    },
                    {
                        ...
                        callback = function()
                            UIManager:show(InfoMessage:new{
                                text = _([[Download directory: use a directory that is exclusively used by the WallabagX plugin. Existing files in this directory risk being deleted.
...
The 'Synchronize remotely deleted files' option will remove local files that no longer exist on the server.]])
            ...
            },
            {
                ...
                callback = function()
                    UIManager:show(InfoMessage:new{
                        text = T(_([[WallabagX is an open source read-it-later service. This plugin synchronizes with a WallabagX server.
...
function WallabagX:getBearerToken()
    ...
    if result then
        ...
        UIManager:show(InfoMessage:new{
            text = _("Could not login to WallabagX server."), })
        return false
    end
end
...
function WallabagX:getArticleList()
    local filtering = ""
...
function WallabagX:filterIgnoredTags(article_list)
    ...
    for _, article in ipairs(article_list) do
        ...
        for _, tag in ipairs(article.tags) do
            if ignoring[tag.label] then
                skip_article = true
                logger.dbg("WallabagX: ignoring tag", tag.label, "in article",
...
--- Download WallabagX article.
-- @string article
-- @treturn int 1 failed, 2 skipped, 3 downloaded
function WallabagX:download(article)
    local skip_article = false
    ...
    -- All webpages are HTML. Ignore them since we want the WallabagX EPUB instead!
    ...
    logger.dbg("WallabagX: DOWNLOAD: id: ", article.id)
    logger.dbg("WallabagX: DOWNLOAD: title: ", article.title)
    logger.dbg("WallabagX: DOWNLOAD: filename: ", local_path)
    ...
    if attr then
        ...
        if self.is_dateparser_available then
            ...
            if server_date < attr.modification then
                skip_article = true
                logger.dbg("WallabagX: skipping file (date checked): ", local_path)
            end
        else
            skip_article = true
            logger.dbg("WallabagX: skipping file: ", local_path)
        end
    end
...
function WallabagX:callAPI(method, apiurl, headers, body, filepath, quiet)
    local sink = {}
    local request = {}
    ...
    logger.dbg("WallabagX: URL     ", request.url)
    logger.dbg("WallabagX: method  ", method)

    local code, resp_headers, status = socket.skip(1, http.request(request))
    socketutil:reset_timeout()
    -- raise error message when network is unavailable
    if resp_headers == nil then
        logger.dbg("WallabagX: Server error:", status or code)
        return nil, "network_error"
    end
    if code == 200 then
        if filepath ~= "" then
            logger.dbg("WallabagX: file downloaded to", filepath)
            return true
        else
            ...
            if content ~= "" and string.sub(content, 1,1) == "{" then
                ...
                if ok and result then
                    ...
                    --logger.dbg("WallabagX: result ", result)
                    return result
                else
                    UIManager:show(InfoMessage:new{
        ...
        if filepath ~= "" then
            ...
            if entry_mode == "file" then
                os.remove(filepath)
                logger.dbg("WallabagX: Removed failed download:", filepath)
            end
        elseif not quiet then
            UIManager:show(InfoMessage:new{
                text = _("Communication with server failed."), })
        end
        logger.dbg("WallabagX: Request failed:", status or code)
        logger.dbg("WallabagX: Response headers:", resp_headers)
        return nil, "http_error", code
    end
end

function WallabagX:synchronize()
    ...
    if self.access_token ~= "" then
        ...
        if articles then
            logger.dbg("WallabagX: number of articles:", #articles)
            ...
            for _, article in ipairs(articles) do
                logger.dbg("WallabagX: processing article ID: ", article.id)
...
function WallabagX:processRemoteDeletes(remote_article_ids)
    if not self.is_sync_remote_delete then
        logger.dbg("WallabagX: Processing of remote file deletions disabled.")
        return 0
    end
    logger.dbg("WallabagX: articles IDs from server: ", remote_article_ids)
    ...
    for entry in lfs.dir(self.directory) do
        if entry ~= "." and entry ~= ".." then
            ...
            if not remote_article_ids[ id ] then
                logger.dbg("WallabagX: Deleting local file (deleted on server): ", entry_path)
...
function WallabagX:processLocalFiles(mode)
    if mode then
        if self.is_auto_delete == false and mode ~= "manual" then
            logger.dbg("WallabagX: Automatic processing of local files disabled.")
            return 0, 0
        end
    end
...
function WallabagX:addArticle(article_url)
    logger.dbg("WallabagX: adding article ", article_url)
...
function WallabagX:addTags(path)
    logger.dbg("WallabagX: managing tags for article ", path)
    ...
    if id then
        ...
        if tags and tags ~= "" then
            logger.dbg("WallabagX: sending tags ", tags, " for ", path)
            ...
            logger.dbg("WallabagX: no tags to send for ", path)
        end
    end
end

function WallabagX:removeArticle(path)
    logger.dbg("WallabagX: removing article ", path)
    local id = self:getArticleID(path)
    if id then
...
function WallabagX:deleteLocalArticle(path)
    if lfs.attributes(path, "mode") == "file" then
        FileManager:deleteFile(path, true)
   end
end

function WallabagX:getArticleID(path)
    -- extract the WallabagX ID from the file name
    ...
    if path:sub(offset , offset + prefix_len - 1) ~= article_id_prefix then
        logger.warn("WallabagX: getArticleID: no match! ", path:sub(offset , offset + prefix_len - 1))
        return
    end
    local endpos = path:find(article_id_postfix, offset + prefix_len)
    if endpos == nil then
        logger.warn("WallabagX: getArticleID: no match! ")
        return
    end
...
function WallabagX:refreshCurrentDirIfNeeded()
    if FileManager.instance then
        FileManager.instance:onRefresh()
    end
end

function WallabagX:setFilterTag(touchmenu_instance)
...
function WallabagX:setTagsDialog(touchmenu_instance, title, description, value, callback)
...
function WallabagX:editServerSettings()
    local text_info = T(_([[
Enter the details of your WallabagX server and account.
...
function WallabagX:editClientSettings()
    self.client_settings_dialog = MultiInputDialog:new {
        title = _("WallabagX client settings"),
        fields = {
            {
...
function WallabagX:setDownloadDirectory(touchmenu_instance)
    require("ui/downloadmgr"):new{
        onConfirm = function(path)
            logger.dbg("WallabagX: set download directory to: ", path)
            self.directory = path
            self:saveSettings()
...
function WallabagX:saveSettings()
    local tempsettings = {
...
function WallabagX:onAddWallabagXArticle(article_url)
    ...
    if wallabag_result then
        UIManager:show(InfoMessage:new{
            text = T(_("Article added to WallabagX:\n%1"), BD.url(article_url)),
        })
    else
        UIManager:show(InfoMessage:new{
            text = T(_("Error adding link to WallabagX:\n%1"), BD.url(article_url)),
        })
    end
...
function WallabagX:getLastPercent()
...
function WallabagX:addToDownloadQueue(article_url)
    table.insert(self.download_queue, article_url)
    self:saveSettings()
end

function WallabagX:onCloseDocument()
